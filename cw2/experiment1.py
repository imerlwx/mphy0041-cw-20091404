...
net = torch.load('task2_saved_model_pt')
net1 = torch.load('task1_label1_saved_model_pt')
net2 = torch.load('task1_label2_saved_model_pt')

## results without the classfication model applied first
diff1 = torch.zeros()
diff2 = torch.zeros()
mean = torch.zeros()
diff = torch.zeros()
...
for i in range(40):
  diff1[i] = (outputs1[i] - labels[i]).sum()
  diff2[i] = (outputs2[i] - labels[i]).sum()
  mean[i] = (diff1[i] + diff2[i]) / 2
  diff[i] = diff1[i] - diff2[i]

diff1 = diff1.detach().numpy()
diff2 = diff2.detach().numpy()
mean = mean.detach().numpy()
diff = diff.detach().numpy()

bland_altman_plot(diff1, diff2)
